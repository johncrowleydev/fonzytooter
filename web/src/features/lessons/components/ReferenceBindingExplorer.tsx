  ...
  return (
    <div className="rounded-lg border border-brand-blue/30 bg-brand-blue/10 p-3">
      <div className="flex items-center justify-between gap-2">
        <span className="text-2xs font-bold uppercase tracking-widest text-brand-blue">
          {label}
        </span>
...
export function ReferenceBindingExplorer() {
  ...
  return (
    <Card className="grid gap-5 p-5">
      ...
      <div
        className={`rounded-lg border p-3 text-xs leading-relaxed ${bindingState.isCopied ? 'border-brand-gold/30 bg-brand-gold/10 text-ink' : 'border-brand-teal/30 bg-brand-teal/10 text-ink'}`}
        role="status"
        aria-live="polite"
      >
        <strong>
      ...
      <div className="grid gap-4 md:grid-cols-3 md:items-stretch">
        <div className="rounded-lg border border-line bg-white/5 p-4">
          ...
          <div className="mt-3 grid gap-3">
            <div className="flex items-center justify-between gap-3 rounded-md border border-line px-3 py-2">
              ...
              <span className="text-xs text-brand-teal" aria-hidden="true">
                → object A
              </span>
            </div>
            <div className="flex items-center justify-between gap-3 rounded-md border border-line px-3 py-2">
        ...
        <div className="grid place-items-center text-2xl text-brand-teal" aria-hidden="true">
          →
        </div>
      ...
      <div className="flex flex-wrap gap-2 border-t border-line pt-4">
        <button
          className={`${controlClass} border-brand-teal/40 bg-brand-teal/10 text-brand-teal hover:bg-brand-teal/20`}
          type="button"
        ...
        <button
          className={`${controlClass} border-brand-blue/40 bg-brand-blue/10 text-brand-blue hover:bg-brand-blue/20`}
          type="button"
        ...
        <button
          className={`${controlClass} border-brand-gold/40 bg-brand-gold/10 text-brand-gold hover:bg-brand-gold/20`}
          type="button"
          onClick={copyBackup}
        ...
        <button
          className={`${controlClass} border-line-strong bg-brand-slate/10 text-ink hover:bg-brand-slate/20`}
          type="button"
